...
// dbDirName and dbFileName place the session-history database inside the
// repository's common git directory, not a per-worktree path: every worktree
// of the same clone shares one git-common-dir, so this is the one location
// naturally shared across worktrees.id rows for the same projects.id. It is
// also never committed (it lives inside .git) and disappears with the repo.
const (
	dbDirName  = "clarity"
